# www -> apex 301 redirect for the youproof.org zone.
#
# A dynamic-redirect ruleset is a ZONE-LEVEL SINGLETON (one entrypoint per phase
# per zone), so it lives here in the single-state org-zone root rather than per
# environment. Mirrors terraform/zone/redirects.tf.
#
...
# hardcoded domains — `www.youproof.org` -> `https://youproof.org` and
# `www.staging.youproof.org` -> `https://staging.youproof.org` both fall out of
# the one rule. The per-environment `www.<domain>` records (cdn root) are what
