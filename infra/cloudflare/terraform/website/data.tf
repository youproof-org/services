# Read the youproof.org zone ID straight from the shared `zone/` root's remote
# state, so this root references the zone WITHOUT a hand-copied ID. Mirrors
# worker/data.tf (which reads the same state for the youproof.hu zone). This
# requires:
...
# enforces the apply order (zone before website).
#
# The `zone/` root exposes TWO zone IDs (`zone_id` = youproof.hu,
# `org_zone_id` = youproof.org); this root reads `org_zone_id`.
