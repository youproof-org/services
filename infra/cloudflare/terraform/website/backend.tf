...
#
# The bucket and per-environment state `key` are NOT set here because a single
# backend block can't be parameterized per environment. They are supplied at
# init time via `-backend-config` (see README and the CI workflow), e.g.:
#
#   terraform init \
#     -backend-config="bucket=youproof-tfstate" \
#     -backend-config="key=cloudflare/website/production.tfstate" \
...
#
# Using a distinct `key` per environment (cloudflare/website/production.tfstate /
# cloudflare/website/staging.tfstate) guarantees the two environments never share
# or clobber state — mirrors the worker root's per-env key convention.
terraform {
  backend "s3" {
    region = "auto"
