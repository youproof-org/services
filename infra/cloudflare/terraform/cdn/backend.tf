...
#     -backend-config="key=cloudflare/cdn/production.tfstate" \
...
#
# Using a distinct `key` per environment (cloudflare/cdn/production.tfstate /
# cloudflare/cdn/staging.tfstate) guarantees the two environments never share or
# clobber state — mirrors the worker root's per-env key convention.
terraform {
  backend "s3" {
    region = "auto"
