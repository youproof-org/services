...
#     -backend-config="key=cloudflare/worker/production.tfstate" \
...
#
# Using a distinct `key` per environment (production.tfstate / staging.tfstate)
# guarantees the two environments never share or clobber state.
terraform {
  backend "s3" {
    region = "auto"
