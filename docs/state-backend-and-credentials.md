...
| `GOOGLE_SITE_VERIFICATION` | var | **Production only.** Google Search Console / GA4 apex verification token (no `google-site-verification=` prefix — the Terraform resource adds it). A var, not a secret: published in public DNS the moment it applies. Unset on staging, which creates no record. |
| `BING_SITE_VERIFICATION` | var | **Production only.** Bing Webmaster Tools verification host label, e.g. `3dab19…` — the bare label, no `.youproof.org` suffix. Same var-not-secret reasoning. |
| `GOOGLE_SITE_VERIFICATION_RECORD_ID` / `BING_SITE_VERIFICATION_RECORD_ID` | var | **Production only, one-time migration inputs.** Cloudflare record ids of the two pre-existing verification records, so `website/imports.tf` adopts them instead of creating duplicates. Must be set together with the matching token (a lifecycle precondition fails the plan otherwise). Removable once the adoption has applied. See [DNS & TLS](dns-and-tls.md#youprooforg-zone). |
| `GA_MEASUREMENT_ID` | var | Google Analytics 4 measurement id (`G-…`) for **this environment's** property — production and staging are separate GA4 properties, so this is one name with a different value per Environment. Consumed by the website build as `NEXT_PUBLIC_GA_MEASUREMENT_ID` and inlined into the static export. Public by design, hence a var (not a secret). **Empty is the kill switch**: with no id the consent banner and analytics are both absent entirely (see [analytics & consent](analytics-and-consent.md#version-0-is-the-kill-switch)). |
